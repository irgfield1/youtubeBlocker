    ...
    browser.tabs.query({ active: true }).then(async (tabs) => {
        ...
        if (present) {
            ...
            if (youtubeVideoPattern.test(tempUrl)) {
                // console.log(youtubeVideoPattern);
        ...
        await readLocalStorage(requestInfo.originUrl)
            .then(data => {
                if (data == "block") {
                    ...
                    reject({ type: "direct" });
                } else if (data.hasOwnProperty(requestInfo.originUrl)) {
                    console.log(data[requestInfo.originUrl]);
...
function whitelistProxyHandler(requestInfo) {
    return new Promise(async (resolve, reject) => {
        ...
        await readLocalStorage(requestInfo.originUrl)
            .then(data => {
                if (data == "block") {
                    reject({ type: "http", host: "127.0.0.1", port: 65535 });
...
//Promises. If I reject one promise, how exactly does that propagate through a system? will promises propagate the way I want them to?


//TODO Whitelist and Blacklist still don't work appropriately